-- Fix all tables: Update courses table and create missing tables
-- Run this SQL in your Supabase SQL Editor

-- ============================================
-- FIX COURSES TABLE
-- ============================================
-- Check if courses table exists and has wrong structure, then fix it
DO $$
BEGIN
  -- Check if courses table exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'courses') THEN
    -- Check if it has the old structure (has 'title' column instead of 'name')
    IF EXISTS (
...
-- Create courses table with correct structure (only if it doesn't exist)
DO $$
BEGIN
  ...
  ELSE
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'name') THEN
      ...
      -- If title exists, copy it to name
...
-- ============================================
-- CREATE MISSING TABLES FROM FEATURES MIGRATION
-- ============================================
